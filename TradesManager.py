...
class TradesManager(threading.Thread):
    ...
    def _swap(self, in_token_address, out_token_address, amount, slippage, priority_fee: int, confirm_transaction):
        ret_val = None
        swap_transaction = TokensApi.get_swap_transaction(self.signer_pubkey, in_token_address, out_token_address, amount, slippage, priority_fee)
        ...
        if confirm_transaction and transaction_checker.wait_for_success(timeout=35):
            ret_val = tx_signature       
